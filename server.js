...
app.post('/api/enrich', auth, async (req, res) => {
  ...
  try {
    ...
    // Build multiple targeted search queries — don't rely on visiting the website directly
    // because most small business sites block bots. Instead, search Google for the email.
    const searches = [
      `"${bizName}" email`,
      `"${bizName}" contact email ${address ? address.split(',')[0] : ''}`,
      domain ? `"@${domain}"` : null,
      domain ? `site:${domain} email OR contact OR mailto` : null,
      `"${bizName}" companies house`,
      `"${bizName}" facebook contact`,
    ].filter(Boolean).join('\n');

    const prompt = `You are a professional business contact researcher specialising in finding UK business email addresses.
...
YOUR TASK: Run multiple web searches to find contact details. Do NOT just try to visit the website — many block bots. Instead use these search strategies:

SEARCH 1: ${domain ? `"@${domain}"` : `"${bizName}" email`}
(This finds any page on the web that mentions their email address)

SEARCH 2: "${bizName}" email contact ${address ? address.split(',')[0] : ''}
(General search for their contact details)

SEARCH 3: ${domain ? `site:${domain} contact` : `"${bizName}" contact page`}
(Look for their contact page specifically)

SEARCH 4: "${bizName}" site:facebook.com
(Find their Facebook page which often has email)

SEARCH 5: "${bizName}" companies house
(UK Ltd companies list directors — often reveals owner name)

CRITICAL EMAIL RULES:
- Run ALL 5 searches above before giving up
- Any text with @ in it is a potential email — grab it
- mailto: links contain emails — check these carefully
- Even if the website blocks you, Google's cache and other sites may have their email indexed
- If you find @${domain || 'theirdomain.com'} anywhere on the web, that is almost certainly their email
- Do NOT say "website cannot be read" — instead run more searches
- Only return emails you actually found — do not invent them

Return ONLY a valid JSON object, no other text:
{
  "emails": ["found@email.com"],
  "owner_name": "Full Name or null",
  "job_title": "Owner/Director or null",
